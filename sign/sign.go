...
/*
isUpperCase:
1.default is upperCase
2.false is upperCase
3.true is lowerCase
**/
func GetMD5Hash(text string, isLowerCase ...bool) (string, error) {
	hasher := md5.New()
	if _, err := hasher.Write([]byte(text)); err != nil {
		return "", err
	}
	if len(isLowerCase) > 0 && isLowerCase[0] == true {
		return fmt.Sprintf("%x", hasher.Sum(nil)), nil
	}
	return fmt.Sprintf("%X", hasher.Sum(nil)), nil
}

func MakeMd5Sign(url, key string, isLowerCase ...bool) (sign string) {
	str := url + "&key=" + key
	sign, _ = GetMD5Hash(str, isLowerCase...)
	return
}

func MakeMd5PureSign(url, key string, isLowerCase ...bool) (sign string) {
	str := url + key
	sign, _ = GetMD5Hash(str, isLowerCase...)
	return
}
